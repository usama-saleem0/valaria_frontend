...
const Dash6ah = () => {
  ...
  const fetchUserData = async () => {
    try {
      const response = await axios.get(`${baseurl}/employeeusers`);
                ...
                <tr key={user.id}>
                  ...
                  <td className="gg-td">
                    <div className="admin-dp">
                      <img src={user.Profile && user.Profile.image ? user.Profile.image : "admin"} alt="" />
                    </div>
                    {user.username}
                  </td>
                  <td>{user.type}</td>
                  ...
                  <td>{user.verified_account === 1 ? 'Verified' : 'Not Verified'}</td>
